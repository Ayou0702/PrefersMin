    ...
    <div class="flex">
      ...
      <div class="flex ml-auto justify-content-end">
        <div class="p-inputgroup">
          <Button
            v-if="urlList['wallpaper'].length > 0"
            label="壁纸"
            type="button"
            @click="updateUrlList('wallpaper')"
          />
          <Button
            v-if="urlList['avatar'].length > 0"
            label="头像"
            type="button"
            @click="updateUrlList('avatar')"
          />
          <Button
            v-if="urlList['liveWallpaper'].length > 0"
            label="动态壁纸"
            type="button"
            @click="updateUrlList('liveWallpaper')"
          />
          <Button
            v-if="urlList['backgroundImage'].length > 0"
            label="背景图"
            type="button"
            @click="updateUrlList('backgroundImage')"
          />
          <Button
            v-if="urlList['toolPlay'].length > 0"
            label="工具玩法"
            type="button"
            @click="updateUrlList('toolPlay')"
          />
          <Button
            v-if="urlList['ai'].length > 0"
            label="AI玩法"
            type="button"
            @click="updateUrlList('ai')"
          />
        </div>
      </div>
    </div>
